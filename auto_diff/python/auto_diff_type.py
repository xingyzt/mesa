...
class AutoDiffType:
    ...
    def __init__(self, name, array, array_length, partials):
        ...
        self.partials = sorted(list(partials),
                               key=lambda p: [p.net_order, tuple(-o for o in p.orders)])
        self.max_order = max(p.net_order for p in self.partials)

        self.unary_partials = sorted(list(Partial((i,), False) for i in range(self.max_order + 1)),
                                     key=lambda p: [p.net_order, tuple(-o for o in p.orders)])
    ...
    def declare_name(self, ref=None):
        ...
        not self.array_length is None):  # Either no array type or else it's a fixed length.
            return 'type(' + str(self.name) + ')'
        else:
            # For variable-length array types we have to know the length at declare-type or else declare the length as deferred.
            # To declare it as referred pass ref='*'. Otherwise pass 'x' (or 'y', 'z', etc.) to specify which
    ...
    def type(self):
        ...
        variables = []
        if self.array and self.array_length is None:
            variables.append('integer, len :: n ! Length of array of independent variables')
        ...
        for p in self.partials:
            ...
            if 'Array' in p_name:
                if self.array_length is None:
                    ...
                    variables.append('real(dp) :: ' + p_name + '(' + str(self.array_length) + ')')
            else:
    ...
    def partial_str_in_instance(self, instance_name, p):
        """
        Returns a string representing the specified partial derivative as a member of an instance of this auto_diff type.
        """
        s = str(p)
        s = instance_name + '%' + s
        if 'Array' in s:
    ...
    def assignment_routine_from_self(self):
        ...
        routine_body = ['this%' + str(p) + ' = other%' + str(p) for p in self.partials]
    ...
    def assignment_routine_from_real_dp(self):
        ...
        routine_arguments = [('this', self.declare_name(ref='*'), 'out'),
                             ('other', 'real(dp)', 'in')]
        routine_body = ['this%val = other'] + ['this%' + str(p) + ' = 0_dp' for p in self.partials
                                               if p.net_order > 0]
    ...
    def comparison_function_with_self(self, operator_name, operator):
        """
        Makes a function for comparing two objects of this type using the specified operator.
        """
    ...
    def comparison_function_with_real_dp(self, operator_name, operator):
        """
        Returns functions for comparing an object of this type with a real(dp) and vice-versa.
        ...
        first_function = FortranFunction(function_name, function_arguments, function_result,
                                         function_body)
        ...
        second_function = FortranFunction(function_name, function_arguments, function_result,
                                          function_body)
    ...
    def comparison_function_with_int(self, operator_name, operator):
        """
        Returns functions for comparing an object of this type with an integer and vice-versa.
    ...
    def derivative_function(self, variable_index):
        """
        Returns a function which takes a derivative with respect to the specified variable.
        """

        function_name = 'differentiate_' + self.name + '_' + str(variable_index + 1)
        function_arguments = [('this', self.declare_name(ref='*'), 'in')]
        ...
        for p in self.partials:
            ...
            if p_increment in self.partials:  # The higher-order derivative exists.
    ...
    def generic_unary_operator_function(self):
        """
        Returns a function which implements the chain rule on a general unary operator.
        """

        function_name = 'make_unary_operator'
        function_arguments = [('x', self.declare_name(ref='*'), 'in')]
        function_arguments = function_arguments + [
            ('z_' + str(p).replace('val1', 'x'), 'real(dp)', 'in') for p in self.unary_partials]
        function_result = ('unary', self.declare_name(ref='x'))
        function_body, function_declarations = \
            unary_generic_chain_rule(self, fixed_length=self.array_length)
    ...
    def generic_binary_operator_function(self):
        ...
        function_arguments = function_arguments + [
            ('z_' + str(p).replace('val1', 'x').replace('val2', 'y'), 'real(dp)', 'in') for p in
            self.binary_partials]
        function_result = ('binary', self.declare_name(ref='x'))
        function_body, function_declarations = \
            binary_generic_chain_rule(self, fixed_length=self.array_length)
        function_body = function_declarations + function_body
    ...
    def specific_binary_operator_function_self(self, operator_name, operator):
        ...
        function_result = ('binary', self.declare_name(ref='x'))
        function_body, function_declarations = \
            binary_specific_chain_rule(self, operator, fixed_length=self.array_length)
        function_body = function_declarations + function_body

        return FortranFunction(function_name, function_arguments, function_result, function_body)

    def specific_binary_operator_function_real_dp(self, operator_name, operator):
        """
        ...
        function_arguments = [('x', self.declare_name(ref='*'), 'in'), ('y', 'real(dp)', 'in')]
        ...
        function_result = ('unary', self.declare_name(ref='x'))
        function_body, function_declarations = \
            unary_specific_chain_rule(self, unary_operator, fixed_length=self.array_length)
        function_body = function_declarations + function_body
        function_1 = FortranFunction(function_name, function_arguments, function_result,
                                     function_body)

        # (real, this)
        function_name = operator_name + '_real_self'
        function_arguments = [('z', 'real(dp)', 'in'), ('x', self.declare_name(ref='*'), 'in')]
        ...
        function_body, function_declarations = \
            unary_specific_chain_rule(self, unary_operator, fixed_length=self.array_length)
        function_body = function_declarations + function_body
        function_2 = FortranFunction(function_name, function_arguments, function_result,
                                     function_body)
    ...
    def specific_binary_operator_function_int(self, operator_name, operator):
        ...
        function_arguments = [('x', self.declare_name(ref='*'), 'in'), ('y', 'integer', 'in')]
        ...
        function_arguments = [('z', 'integer', 'in'), ('x', self.declare_name(ref='*'), 'in')]
