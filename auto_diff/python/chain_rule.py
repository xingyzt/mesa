...
def unary_generic_chain_rule(auto_diff_type, fixed_length=None):
    ...
    def operator(x):
        ...
        # respect to the x.
        z = sum(sym * x ** p.orders[0] / factorial(p.orders[0]) for sym, p in
                zip(*(z_syms, auto_diff_type.unary_partials)))
        return z
...
def binary_generic_chain_rule(auto_diff_type, fixed_length=None):
    ...
    # Construct the symbols corresponding to the partial derivatives of z.
    # These are d1z, d2z, ..., dNz, giving dz/dx, d^2 / dx^2, and so on.
    z_symbol_strs = ['z_' + str(p).replace('val1', 'x').replace('val2', 'y') for p in
                     auto_diff_type.binary_partials]
    z_symbol_str = ' '.join(z_symbol_strs)
    z_syms = wrap_element(symbols(z_symbol_str, real=True))
    ...
    def operator(x, y):
        ...
        z = sum(sym * x ** p.orders[0] * y ** p.orders[1] / (
                    factorial(p.orders[0]) * factorial(p.orders[1])) for sym, p in
                zip(*(z_syms, auto_diff_type.binary_partials)))
        return z
...
def unary_specific_chain_rule(auto_diff_type, operator, xval=None, fixed_length=None):
    ...
    indep_symbol_str = ' '.join(
        auto_diff_type.partials[0].val_name(i) for i in range(auto_diff_type.num_independent))
    ...
    x_symbol_str = ' '.join(
        auto_diff_type.partial_str_in_instance('x', p).replace(':', 'colon') for p in partials)
    ...
    for p in partials:
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            ...
            d = d.replace(DiracDelta,
                          zero_function)  # Diract Delta is only non-zero in a set of measure zero, so we set it to zero.
            d = d.replace(sign,
                          sgn)  # simplify can do weird things with sign, turning it into the piecewise operator. We want to avoid that so we redirect to a custom sgn function.
            d = d.replace(Derivative,
                          zero_function)  # Eliminates derivatives of the Dirac Delta and Sign, which are non-zero only at sets of measure zero.
            d = d.subs(indep, 0)
...
def binary_specific_chain_rule(auto_diff_type, operator, xval=None, yval=None, fixed_length=None):
    ...
    x_syms = wrap_element(symbols(x_symbol_str, real=True))
    y_symbol_str = ' '.join(
        auto_diff_type.partial_str_in_instance('y', p).replace(':', 'colon') for p in partials)
    ...
    for p in partials:
        # Beginning of Fortran line, for writing out the answer
        binary_symbol_str = auto_diff_type.partial_str_in_instance('binary', p).replace(':',
                                                                                        'colon')
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            d = diff(d, indep, order)
            d = d.replace(DiracDelta,
                          zero_function)  # Dirac Delta is only non-zero in a set of measure zero, so we set it to zero.
            d = d.replace(sign,
                          sgn)  # simplify can do weird things with sign, turning it into the piecewise operator. We want to avoid that so we redirect to a custom sgn function.
            d = d.replace(Derivative,
                          zero_function)  # Eliminates derivatives of the Dirac Delta and Sign, which are non-zero only at sets of measure zero.
