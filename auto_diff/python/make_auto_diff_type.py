...
def make_auto_diff_type(auto_diff_type, unary_operators, binary_operators, comparison_operators,
                        intrinsics):
    """
    ...
    begin = ['use const_def, only: dp, ln10, pi', 'use utils_lib',
             'use support_functions', 'use math_lib']
    begin = begin + ['', 'implicit none', '', 'private', '', '']
    contains = ['contains', '']
    ...
    # Put it all together
    lines = header + indent_list(begin + type_lines + interfaces) + contains + indent_list_of_str(
        functions) + ender
