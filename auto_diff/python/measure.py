...
for i, p in enumerate(powN):
    weights['FUNC_POW' + str(i)] = basic * np.log2(i + 1)
...
def weighted_count_ops(expr_original, verbose=False):
    ...
    if isinstance(components[0],
                  Rational):  # Means there's just one operation and it occurs repeatedly.
        parts = {str(components[1]): components[
