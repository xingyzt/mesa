class Partial:
    ...
    def __hash__(self):
        return hash(self.orders) + hash(self.num_independent) + hash(self.array)
    ...
    def completion_partials(self):
        """
        In order to apply the chain rule with this partial, we need all partials that are one-lower order (min zero).
        """

        # Go through each independent variable and construct the partial that's one-lower order.
        # Skip those where this pushes the order of any independent variable below zero.
        completion = []
