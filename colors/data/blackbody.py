...
def do_one(f_in, vega_interp, teff):
    ...
    j_norm = integrate.romberg(j_f_interp, j_m[0], j_m[-1], divmax=50, vec_func=True, tol=1e-10,
                               rtol=1e-10)
    ...
    def const(t):
        return mbolsun - 2.5 * np.log10(
            (4.0 * np.pi * (10.0 * pc) ** 2 * stefan * t ** 4) / lsun) - magZero
    ...
    def bc(t):
        constants = const(t)
        top = integrate.romberg(topfunc, j_m[0], j_m[-1], args=(t,), divmax=50, vec_func=True,
                                tol=1e-10, rtol=1e-10)
        bot = integrate.romberg(botfunc, j_m[0], j_m[-1], divmax=50, vec_func=True, tol=1e-10,
                                rtol=1e-10)
        print(t, top, bot)
