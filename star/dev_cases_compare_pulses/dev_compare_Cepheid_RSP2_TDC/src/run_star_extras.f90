...
include 'multi_stars_extras.inc'
   ...
   integer function extras_finish_step(id)
      ...
      if (s% id == 2 .and. s% x_logical_ctrl(1)) then
         ...
         do k = 1, s% nz
            ...
            s% mlt_vc(k) = s_other% w(k) * sqrt_2_div_3
   ...
   subroutine data_for_extra_profile_columns(id, n, nz, names, vals, ierr)
      ...
      names(i) = 'vc_R'; i = i + 1
      names(i) = 'vc_diff'; i = i + 1
      names(i) = 'vc_drel'; i = i + 1
      ...
      if (.not. associated(s_other% Y_face)) then
         ...
         do k = 1, nz
            ...
            if (s_other% RSP2_flag) then
               val = s_other% w(k) * sqrt_2_div_3
            else
               val = 0d0
            end if
            vals(k, i) = val; i = i + 1
            vals(k, i) = val - s% mlt_vc(k); i = i + 1
            if (s% doing_first_model_of_run .and. s% x_logical_ctrl(2) .and. &
               abs(vals(k, i - 1)) > 1d0 .and. &
               max(1d0, abs(val), abs(s% mlt_vc(k))) > 1d2 * min(abs(val), abs(s% mlt_vc(k)))) then
               write(*, 3) 'vc diff', k, s% model_number, vals(k, i - 1), val, s% mlt_vc(k)
               call mesa_error(__FILE__, __LINE__, 'data_for_extra_profile_columns')
            end if
            vals(k, i) = rel_diff(val, s% mlt_vc(k)); i = i + 1
