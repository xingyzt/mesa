...
include "test_suite_extras.inc"
   ...
   subroutine extras_controls(id, ierr)
      ...
      if (ierr /= 0) then
         ...
         write(*, *) 'pretend successfully used OP_mono opacities'
      end if
   ...
   subroutine extras_after_evolve(id, ierr)
      ...
      ! running at all is the primary check
      write(*, *) 'successfully used OP_mono opacities'
   ...
   subroutine data_for_extra_profile_columns(id, n, nz, names, vals, ierr)
      ...
      names(1) = 'dlnkap_dlnd'
      do k = 1, nz
         vals(k, 1) = s% d_opacity_dlnd(k) / s% opacity(k)
      end do

      names(2) = 'dlnkap_dlnT'
      do k = 1, nz
         vals(k, 2) = s% d_opacity_dlnT(k) / s% opacity(k)
      end do
