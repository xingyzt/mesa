...
# This makes some fake data for demonstrating MESA's ability to interpolate over arbitrary data in the colors module
    ...
    for i in np.linspace(tmin, tmax, nt):
        for j in np.linspace(gmin, gmax, ng):
            for k in np.linspace(amin, amax, na):
                ...
                print("{:10.4f}".format(10 ** i), "{:10.4f}".format(j), "{:10.4f}".format(k),
                      "{:10.4f}".format(rr), file=f)
