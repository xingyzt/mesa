...
for i in range(1, imax + 1, 1):
    ...
    my.print_parameters(mp, rp, mcore, rhocore, mp_wo_core, irrad_col, flux_dayside, Teq, y, z,
                        maxage)

    createmodel = "planet_create_" + str(mp_wo_core)[0:6] + "_MJ_" + str(rp) + "_RJ.mod"
    coremodel = "planet_core_" + str(mp)[0:6] + "_MJ_" + str(mcore)[0:6] + "_ME_" + str(
        rp) + "_RJ.mod"
    evolvemodel = "planet_evolve_" + str(mp)[0:6] + "_MJ_" + str(mcore)[0:6] + "_ME_" + str(
        rp) + "_RJ.mod"

    if do_create_planet:
        inlist1 = "inlist_create_" + str(mp)[0:6] + "_MJ_" + str(mcore)[0:6] + "_ME_" + str(
            rp) + "_RJ"
    ...
    if do_put_in_core:
        if mcore > 0.0:
            inlist2 = "inlist_core_" + str(mp)[0:6] + "_MJ_" + str(mcore)[0:6] + "_ME_" + str(
                rp) + "_RJ"
    ...
    if do_evolve_planet:
        inlist3 = "inlist_evolve_" + str(mp)[0:6] + "_MJ_" + str(mcore)[0:6] + "_ME_" + str(
            rp) + "_RJ"
        run_time = my.evolve_planet(irrad_col, flux_dayside, maxage, inlist3, coremodel,
                                    evolvemodel)
